...
def process_image(train_noisy,patch_size=2):
    # 按6x6的小块计算训练图像像素的标准差
    STD_train = []
    for h in range(3,train_noisy.shape[1]-patch_size):
        for w in range(3,train_noisy.shape[2]-patch_size):
            patch = train_noisy[:,h-patch_size:h+patch_size+1,w-patch_size:w+patch_size+1,:]/255
            reshaped_patch = patch.reshape([-1, (patch_size*2+1), 3])
            std_values = np.std(reshaped_patch, 1)
            STD_train.append(std_values.reshape([-1,1,3]))   
